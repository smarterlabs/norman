module.exports = async function fetchProducts(norman, client, options, page = 1, previousFetch) {
	let products
	let newProducts
	try {
		if (!previousFetch) {
		...
		console.error(err)
		setTimeout(() => fetchProducts(norman, client, options, page, previousFetch), options.timeout)
		return
	}
	...
	if (lastProd.hasNextPage && (!options.pageLimit || page <= options.pageLimit)) {
		await fetchProducts(norman, client, options, page, previousFetch)
	}
